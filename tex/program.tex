...
\subsection{O1--O8: LPS Phase --- Cascade Exponent \texorpdfstring{$\beta$}{beta}}
  ...
  \subsection{O25--O30, PTO, and Fibre Erasure: Numerical Validation, Carrier Diagnostics,
    ...
    \textbf{O27}~\cite{Beau2026a31}: Quotient factorisation alone does not imply equivariance. Saturation, the supplied
    carrier, and parity
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    isomorphism). The identification $H_{\mathrm{eff}}\cong\mathrm{Sym}^2(V_\rho)$ that it imports
    from O28--O29 is not supplied by O29 version~2.0, so the typed bridge and its uniqueness are
    conditional on a supplied identification; the paper reduces the
    ...
    (Q5b Theorem~3.2).
    (\sstruct{} for the isotropy calculations; the typed symmetric-square bridge and its uniqueness are
    conditional on the supplied identification).
    ...
    (\sproved{} as a conditional implication; version 1.2).
    ...
    defect-rate identity (\sstruct). For every smooth family and every smooth unitary transport the projector jet
    ...
    not being excluded; an explicit covariant moving-Schur family realises $E_0^2 = \mathrm{diag}(1, \tfrac12,
    ...
    with an ambient doubling, a canonical inter-sector identification and the condensation dynamics left open.
    ...
    \begin{figure}[p]
      ...
      \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[
          ...
          \node[S] (O27)  at (6*\dx,-6*\dy)   {O27};
          \node[S] (O28)  at (7*\dx,-6*\dy)   {O28};
          \node[S] (O29)  at (7*\dx,-7*\dy)   {O29};
          \node[S] (O30)  at (7*\dx,-8*\dy)   {O30};
          \node[font=\tiny\sffamily,text=cstruct,right=2pt] at (O30.east)
          ...
          \draw[arr]   (O28) to[out=0,in=0] (O30);
          \draw[arr]   (O31) -- (O32);
          \draw[arr]   (O25) -- (O26);
          \draw[arr]   (O25) to[out=-30,in=-150] (O27);
          \draw[arr]   (O28) to[out=0,in=0] (O30);
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      ...
      No Mosco derivation of $L_\Pi=-A\partial_x^2$; the spatial limit is the hypothesis [H-L]; Q5 open &
      ...
      that identification is not supplied by O29 version 2.0, so the typed bridge and its uniqueness
      are conditional on it &
      \sstruct{} (isotropy); conditional (bridge) \\[3pt]
      ...
      chiral lift: the residual frequency reflection is the central Weil lift $R_b = W(-I) = \mathrm{FT}^2$,
      hence $[\gamma_5, R_b] = 0$ &
      \sstruct \\[3pt]
      ...
      no new result &
      \ssynth \\[3pt]
      NIF\-Note & The non-injective foundations sub-programme (Note~5, version 1.4) & \cite{Beau2026ninote} &
      Maps ENI, Foundation, HeisenbergStructure and noscale into the audited axiomatic chain:
      A1--A4 $\Rightarrow$ $\Pi$ non-injective, while the non-trivial commutator does not force
      $\Heis_3(\mathbb{Z}/q\mathbb{Z})$ (the finite $S_3$ countermodel is cited there from the retired
      HCO record, since consolidated into HeisenbergStructure 2.1); the Heisenberg/Weil carrier is a
      supplied realisation.
      ...
      monotonicity along the cascade; version 1.2 integrates the ENT-E1 and ENT-E2 results, the
  ...
  \paragraph{$V-A$ chiral structure and fermionic matter (Q14).}
    ...
    A qualitative mechanism for the inter-generation splitting is identified (Q14~\S6:
  ...
  The Cosmochrony programme has organised a coherent, multi-level programme toward the derivation of
