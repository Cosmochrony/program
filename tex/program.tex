...
\begin{document}
  ...
  \begin{abstract}
    ...
    Heisenberg group $\Heis_3(\mathbb{Z}/q\mathbb{Z})$ and its Weil representation as theorems,
    without any background substrate or relaxation mechanism.
    Branch~II (O-series) develops the spectral admissibility sub-programme, deriving and
    numerically validating the transfer chain $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to
    \beta^* \approx 0.126$ that connects the Born--Infeld saturation constant to the
    charged-lepton mass hierarchy, and identifies the Standard Model gauge group
    $G_{\mathrm{SM}} = \SU(3)\times\SU(2)\times\mathrm{U}(1)$ (unconditional at the pointwise
    level on the standard graph).
    Branch~III (Q-series and companion papers) derives quantum mechanics, spacetime geometry, and
    gauge dynamics from the Branch~I axioms and Branch~II spectral data: the universal singlet
    correlator and Born rule (paper Q3), the effective Lorentzian co-metric
    $g^{\mu\nu} = \mathrm{diag}(-2,2,2,2) \propto \eta^{\mu\nu}$ under the continuum-limit
    hypotheses (the geometric-emergence papers, with $A_\tau = 2$ from paper Q11), and the
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
  ...
  and claims still pending --- notably the Q13 gauge--gravity hierarchy and the
  $8\pi G_N = c_{\mathrm{YM}}/c_{\mathrm{EH}}$ normalisation --- are marked as such.
