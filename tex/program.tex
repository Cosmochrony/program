...
\begin{document}
  ...
  \sim (12\cdot 16\pi^2)^{-1}\log(\Lambda/\mu)$,
    ...
    coefficient, whose finite normalisation remains a matching datum), recasts it as a conditional
    variational stationarity, extends it causally to gravitational-wave propagation, and provides its
    ultraviolet Born--Infeld completion.
  ...
  \subsection{Gravity and Spacetime Track}
    ...
    \textbf{Gravity}~\cite{Beau2026h} (v3.3, retitled \emph{Conditions for an Infrared Einstein Sector from
    Spectral Geometry}): Determines the conditions under which the metric variation of
    $S_\Pi[g] = \tfrac{1}{2}\log\det' A_g$ supports a local infrared Einstein sector. Version~3.3 separates two
    regularisation statements that earlier versions conflated: a physical proper-time cutoff
    ...
    tensorial completion of a supplied Einstein infrared term remains of determinantal Born--Infeld form
    $\sqrt{-\det(g_{\mu\nu}+\ell_{\mathrm{sp}}^2 R_{\mu\nu})}-\sqrt{-g}$ (\sstruct).

    \textbf{Thermodynamics}~\cite{Beau2026Thermodynamics} (v2.0, retitled \emph{Einstein--Matter Coupling from
    Spectral Stationarity: A Conditional Variational Result and the Open Thermodynamic Bridge}):
    Version~2.0 \emph{withdraws} the thermodynamic identifications claimed up to v1.1 and retains only a
    conditional variational core. Assuming a renormalised infrared metric response (supplied by Gravity~v3.3,
    ...
    and the earlier multiplier field $\beta(x)^{-1}$ and spectral first law are retracted. A thermodynamic
