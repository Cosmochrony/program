...
\date{2026\\[2pt]\small Version 2.9}
...
\begin{document}
  ...
  \subsection{O16--O24: Pair Observable and the Fibre-Level Structure Programme}
    ...
    \textbf{O18}~\cite{Beau2026a22}: Born--Infeld parity and an open fibre bridge.
    The evenness $S_{\mathrm{BI}}[-\chi]=S_{\mathrm{BI}}[\chi]$ is elementary, but it does not
    imply $\Pi(\chi)=\Pi(-\chi)$.  The paper's perturbative indiscernibility proof compares
    opposite perturbations rather than the same perturbation, and no map deriving
    $\chi\mapsto-\chi$ as $c\mapsto q-c$ is supplied
    (\sproved{} for action parity; fibre and Weil identifications: \sopen).
    ...
    the pipeline-independence proof is incomplete (generic linear algebra: \sproved;
    canonical observable: \sopen).
    ...
    itself are open; the numerical interval $[7.4,10.6]$ is not derived (\sstruct).
    ...
    an exact power law and a fixed normalisation.  It is not invariant under
    $\sigma\mapsto\lambda\sigma$: it scales as $\lambda^{1/\delta}$, so it does not eliminate
    amplitude or pipeline dependence.  The threshold $9/q^2$, shell-alignment condition, and
    cross-substrate carrier remain external inputs; SGN excludes a native Heisenberg transfer
    for the corpus-defined objects (local algebra: \sproved; intrinsic rank and transfer:
    \sopen).
    ...
    close O21's intrinsic-definition problem (\sproved{} within the stated model).
  ...
  \subsection{O25--O30, PTO, and Fibre Erasure: Numerical Validation, Sector Identification, Eigenvalue Ratio, and Information-Theoretic Axes}
    ...
    Under the proposed Born--Infeld/Weil parity identification of O18,
    $\pi_{q-c}(v) = \overline{\pi_c(v)}$ is anti-linear, so every outer
  ...
  \subsection{Q-Series: Quantum Mechanics and Spacetime Emergence}
    ...
    \textbf{Q1}~\cite{Beau2026q1}: Proposed phase-coherence route from projective admissibility.
    Its identification of conjugate Weil blocks with a BI-indiscernible fibre imports O18's open
    parity-to-fibre and substrate-to-character bridges.  The published central proof also requires
    an independent audit of its transition and rank claims; the resulting singlet and Tsirelson
    chain is therefore not unconditional (\sstruct{} under open upstream premises).
  ...
  sectors of~$2I$).
    ...
    admissibility thread; O18, O22, O23, BornInfeld for the proposed
    parity/isotropy/indiscernibility inputs; ENI for the Bell argument), records the
    proved/numerical/open status of every
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      ...
      pipeline-independent observable are not established for the real pipeline &
      \sopen \\[3pt]
      ...
      within the stated model; this does not make that rank amplitude-invariant &
      \sproved\ /\ \sopen \\[3pt]
