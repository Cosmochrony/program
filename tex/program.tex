...
\date{2026\\[2pt]\small Version 2.17}
...
\begin{document}
  ...
  \paragraph{Conventions on result status.}
    ...
    formalised);
    \snum{} (reported measurements or exact finite computations that establish no theorem beyond the
    data they report);
    \scond{} (a theorem-level result whose load-bearing input is a supplied identification or an
    unestablished hypothesis of the programme, so that the result cannot be read as established until
    that input is);
    ...
    the papers they map.
    Each paper carries one primary label, stated first in the inventory table and in the dependency
    figures and graph, followed where useful by the labels of its secondary components; the primary
    label is the status of the paper's principal contribution.
...
\subsection{O1--O8: LPS Phase --- Cascade Exponent \texorpdfstring{$\beta$}{beta}}
  ...
  \subsection{Cosmological and Condensed-Matter Applications}
    ...
    (\sstruct{} for the re-audited $8/3$ fibre-to-base stiffness; capacity ceiling: conditional theorem chain
...
% and the Q8--Q10/U1/W1 block).
          ...
          \node[Oc] (Grav) at (-1.5*\dx,-6*\dy) {Gravity};
          \node[S] (Thm)  at (-1.5*\dx,-7*\dy) {Thermo};
          \node[Oc] (Lor)  at (-1.5*\dx,-8*\dy) {Lorentz};
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
      ...
      Colour coding: \textcolor{cproved}{$\blacksquare$}~\sproved{},
        ...
        \textcolor{csynth}{$\blacksquare$}~\ssynth{}; each node carries the primary label of its inventory row.
