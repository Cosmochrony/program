...
\date{2026\\[2pt]\small Version 2.15}
...
\begin{document}
  ...
  \subsection{O25--O30, PTO, and Fibre Erasure: Numerical Validation, Sector Identification, Eigenvalue Ratio, and Information-Theoretic Axes}
    ...
    co-admissibility and gauge-group claims (\sopen).
    ...
    limit. O31 version~2.0 withdraws the claimed pointwise proof and places the associated
    scalar material under re-audit. The O32 numerical values remain reported measurements, but they
    do not establish individual-profile co-admissibility or identify a gauge group.
    Status: \textbf{numerical} for the reported measurements; their former O31-based structural interpretation
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    the projective spectral entropy (\sstruct, conditional on a supplied compact gauge group;
    O31 no longer identifies its $\SU(3)$ factor).
    ...
    on Q12, the Gravity paper and a supplied compact gauge group; O31 no longer identifies
    an $\SU(3)$ factor).
    ...
    on Q13, the Gravity paper, the BI paper and a supplied gauge representation; O31 no
    longer makes the colour sector unconditional).
    ...
    \begin{figure}[p]
      ...
      \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[
          ...
          \node[font=\tiny\sffamily,text=copen,right=2pt] at (O31.east)
            {withdrawn claims};
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      ...
      metaplectic step, $\Delta(27)$ proposition, rank-eight test, $\SU(3)$ uniqueness and
      every gauge-group derivation; gives explicit $\mathrm{SO}(3)$, $T^2$ and
      ...
      pointwise interpretation and gauge-group reading withdrawn by O31 version~2.0 &
      \textbf{numerical} / \sopen \\[3pt]
