...
\subsection{O1--O8: LPS Phase --- Cascade Exponent \texorpdfstring{$\beta$}{beta}}
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    algebra isomorphism), and, \emph{under an explicit identification hypothesis} that no source
    supplies, that $\mathrm{Sym}^2(V_\rho)$ is the unique irreducible three-dimensional complex
    $\mathfrak{su}(2)$-module, that an equivariant map to the complexified adjoint is either zero or
    unique up to one non-zero complex scalar (a real positive normalisation being a further
    convention), and that the pullback of the Hermitian spatial form is graded, with
    $J_3$-invariance forcing isotropy within the horizontal plane only.
    Proves analytically $[F_c, L_{\mathrm{Weil}}] = 0$; version~2.0 withdraws the transfer of that
    statement to the cross terms of $\sigma_2(L_{\mathrm{eff}})$, which requires the missing
    identification.
    Its numerical section is re-typed: the three stored basis rows of the O25 checkpoints are pure
    Fourier modes, so every entry of the compression is the Rayleigh quotient
    $4 - 2\cos(2\pi m/q)$ of $L_{\mathrm{Weil}} = 4I - W_a - W_a^\dagger - W_b - W_b^\dagger$.
    Hence $A_z = 4-2-0 = 2$ is the flat-mode value, not a Casimir signature and not
    normalisation-invariant; $A_H - 2 = 4\sin^2(\pi k/q)$ is an identity, so the vanishing of the
    gap is equivalent to $\mathrm{dist}(k/q,\mathbb{Z}) \to 0$, a property of the pipeline's
    mode-selection rule that the corpus does not derive; and the fit
    $|A_H - A_z| \approx 0.364\,q^{-0.52}$ tracks that rule rather than a spectral rate.
    ...
    Verdict on the bridge: \emph{missing identification} (verdict~3), neither derived nor ruled out;
    ...
    (\sstruct{} for the representation-theoretic requirements on a supplied carrier; the typed
    symmetric-square bridge, its uniqueness and any value of $A_H$ or $A_z$ remain conditional on
    ...
    That reading is withdrawn at its first link: Q7 version~2.0 no longer supplies the
    symmetric-square identification the chain consumes, and it withdraws the coefficient values it
    had imported back from Q8, Q10, U1 and Q11, whose own entries below are pending revision in
    this cascade.
    Until those revisions are made, the chain is to be read as a sequence of conditional steps with
    a missing identification at its head, not as a closure.
  ...
  \paragraph{Physical path.}
    ...
    O23 $+$ O28 $+$ Q5b $\dashrightarrow$ Q7 (proposed dimensional bridge; the identification is
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
      ...
      Colour coding: \textcolor{cproved}{$\blacksquare$}~\sproved{},
        ...
        The dashed uncoloured arrow from Q7 leads to the geometric emergence block Q8--Q10 and U1,
        detailed in Figure~\ref{fig:geom}; it carries no status colour since Q7 version~2.0
        withdraws the values it had imported from that block.
