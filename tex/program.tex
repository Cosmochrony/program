...
\date{2026\\[2pt]\small Version 2.16}
...
\begin{document}
  ...
  Its carrier-selection step is not derived: version~2.1 of
  HeisenbergStructure~\cite{Beau2026HeisenbergStructure} incorporates a finite countermodel that
  ...
  \subsection{A Finite Obstruction to Heisenberg Carrier Selection from Admissibility Constraints}
    ...
    Earlier versions of HeisenbergStructure~\cite{Beau2026HeisenbergStructure}
    provided the published proof attempt for Foundation Theorem~5.6, omitted from the
    Foundation paper for concision. Version~2.1 replaces that claim by an obstruction theorem.
    ...
    Version~2.1 of HeisenbergStructure gives the six-element countermodel
  ...
  sub-programme. Version~1.4 maps the five precursor papers and O1--O33 while separating proved
  mathematics, numerical measurements, conditional models, open bridges, and withdrawn claims.
  It records the failure of the native Heisenberg capacity-to-rate bridge, the conditional scope of
  O27 and O30, O29's numerical-only carrier diagnostics, the O31 withdrawal, O32's numerical-only
  status, and O33's exact finite architecture. Their individual contributions follow.
...
\subsection{O1--O8: LPS Phase --- Cascade Exponent \texorpdfstring{$\beta$}{beta}}
  ...
  \subsection{O25--O30, PTO, and Fibre Erasure: Numerical Validation, Carrier Diagnostics, a Conditional Covariance Model, and Information-Theoretic Axes}
    ...
    \textbf{O27}~\cite{Beau2026a31}: Version~2.0 corrects the factorisation claim: quotient
    factorisation alone does not imply equivariance. Saturation, the supplied carrier, and parity
    ...
    The tested conjugate-pair protocol does not reach rank~$4$.
    ...
    $\operatorname{Sym}^2(V_\rho)$ (\textbf{conditional}).
    ...
    O31 version~2.0 places the former scalar and averaged $[H\text{-color}]$ arguments under re-audit;
    they are not retained here as proved conclusions. The O32 numerical values remain reported measurements, but they
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    The chain is conditional at its first link: Q5a version 3.2 withdraws the Mosco derivation
    of the spatial limit operator, which is now the explicit, unestablished hypothesis [H-L]
    (Q5b version 2.1); the geometric convergence results and algebraic rigidities are
    ...
    Version 2.1 formalises the spatial limit input as the explicit, unestablished
    ...
    isomorphism). Its proposed identification
    $H_{\mathrm{eff}}\cong\mathrm{Sym}^2(V_\rho)$ is not supplied by O29, and the present proof
    assumes the typed bridge that it aims to derive.
    The claimed structural uniqueness is therefore under audit; the paper also reduces the
    ...
    (Q5b Theorem~3.2).
    (\sopen; the isotropy calculations survive separately, while the typed symmetric-square bridge
    and its uniqueness require correction).
    ...
    (\sproved{} as a conditional implication; version 1.1).
    ...
    With the additional, independent and unestablished spatial-limit hypothesis [H-L], the
    resulting coefficients have the effective Lorentzian co-metric reading
    $g^{\mu\nu} = \mathrm{diag}(-2, 2, 2, 2) \propto \eta^{\mu\nu}$ (Minkowski).
    ...
    [H-F] and [H-L]} (Q5a version 3.2; Q5b version 2.1).
    ...
    (\sstruct, conditional on a supplied compact gauge group and admissible connection;
    O31 no longer identifies its $\SU(3)$ factor). This fixed-base theorem does not depend on Q11;
    ...
    on supplied geometric and gauge data (\sstruct, conditional on Q12, the Gravity paper,
    matching coefficients and a supplied compact gauge group; O31 no longer identifies an
    $\SU(3)$ factor). Its interpretation on the Cosmochrony emergent base is separately
    ...
    and supplied gauge data; O31 no longer makes the colour sector unconditional).
    ...
    If one additionally supplies an identification of the rank-three admissible sector with
    $\mathrm{Sym}^2(V)$---an identification not derived by O29~\cite{Beau2026a33} or by O30's
    conditional model~\cite{Beau2026a34}---a static $J_\Pi$-real,
    ...
    working-paper version~2.0). FM-Note organises Theorems~A--C into the conditional chain
  ...
  \paragraph{Physical path.}
    ...
    O23 $+$ O28 $+$ O29 $+$ Q5b $\dashrightarrow$ Q7 (proposed dimensional bridge; typed proof open);
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{figure}[p]
      ...
      \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[
          ...
          \node[P] (Q9)  at (\dx,-3*\dy) {Q9\\{\tiny [H-F]$\Rightarrow$[H-lift]}};
      ...
      \textbf{Q9}~(\sproved{} as a conditional implication): under the frozen, unestablished
        ...
        unestablished spatial-limit hypothesis [H-L] of Q5a version 3.2/Q5b version 2.1;
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      ...
      v1.16: continuum-limit passages conditioned on [H-L] (Q5a 3.2 withdrawal); co-metric
      ...
      Version 2.1 incorporates the explicit \(S_3\) countermodel to the carrier-selection
      implication and corrects the central-subgroup/Schur step; the conditional Stone--von Neumann
      ...
      Version 2.0: quotient factorisation does not imply equivariance; under explicit saturation,
      ...
      O28 reports $r_{\mathrm{eff}}^{1\%}=3$ and resolved spectrum
      ...
      former pointwise and averaged interpretations and every gauge-group reading withdrawn
      or placed under re-audit by O31 version~2.0 &
      ...
      v3.2 on a supplied finite-Heisenberg carrier and non-trivial central character: canonical
      ...
      $g^{\mu\nu}=2\,\eta^{\mu\nu}$ determined under stated inputs (Q9, Q10, Q11).
      v2.1: spatial limit input formalised as unestablished hypothesis [H-L]; Q9 proves only
      [H-F] $\Rightarrow$ [H-lift], with [H-F] unestablished; metric results conditional on
      [H-L]; Carnot/$D_{\mathrm{hom}}=4$ geometry independent &
      \sstruct{} (geometry); conditional on [H-F] and [H-L] (metric chain) \\[3pt]
      ...
      Proves $[\mathfrak{su}(2)]\not\cong\mathfrak{heis}_3$ and
      $[F_c,L_{\mathrm{Weil}}]=0$; under the supplied identification
      $H_{\mathrm{eff}}\cong\mathrm{Sym}^2(V_\rho)$, reduces the bridge to isotropy.
      The typed bridge and its claimed uniqueness remain under audit &
      \sopen \\[3pt]
      ...
      any co-metric reading additionally requires [H-L] &
      \sstruct\ /\ \sopen \\[3pt]
      ...
      the bridge. v1.1 result: [H-F] $\Rightarrow$ [H-lift] &
      \sproved \\[3pt]
      ...
      additionally requires the independent, unestablished [H-L] &
      \sstruct\ /\ \sopen \\[3pt]
      ...
      Constrains PRS open deliverable~1, the explicit Lorentzian $1-P(s)$, structurally: for every smooth family and
      every smooth unitary transport the self-adjoint projector jet $1-P(s) = Q_0 + sQ_1 + s^2Q_2 + O(s^3)$ is
      constrained --- $Q_1 = [A, Q_0]$ is a purely off-diagonal Grassmann tangent
      ($Q_0 Q_1 Q_0 = 0 = (1-Q_0)Q_1(1-Q_0)$), $Q_2$ diagonal blocks pinned to $Q_1^2$ with opposite signs; the parity grading holds under $J_\Pi$-equivariance, and with a covariant moving
      spinorial embedding and a $J_\Pi$-even Dirac operator the residue jet is graded $E_0$ even, $E_1$ odd, the
      first-order $J_\Pi$-odd part of $E_\Pi^2$ is $\{E_0, E_1\}$, and
      $u'(0) = \langle J_3, \{E_0, E_1\}\rangle/\langle J_3, J_3\rangle$;
      antiunitary parity makes every odd-order generation-mixing coefficient vanish in every covariant family, with or
      without complex phases (even orders not excluded);
      an explicit covariant moving-Schur family realises $E_0^2 = \mathrm{diag}(1, \tfrac12, \tfrac12)$ with
      $u'(0) = 11 \cdot 2^{1/4}/9 \neq 0$;
      for the parity written in the chiral frame as $\Sigma_\tau \circ \mathrm{conj}$,
      $[\Pi_{J_\Pi\text{-odd}}(1-P)]_{LL} = \tfrac12\Delta_\chi(P)$ and
      $[\Pi_{J_\Pi\text{-odd}}\dot Q(0)]_{LL} = \tfrac12\partial_s\Delta_\chi(P)|_0$ determine one block of the odd
      tangent, not a unique generator;
      selection of the family, localisation of the odd tangent, normalisation of $\partial_s\Delta_\chi(P)|_0$, and
      generation mixing at even orders open, $|u|$ dictionary-bound &
      \sstruct \\[3pt]
      ...
      Version 1.4 maps the five precursor papers and O1--O33 with proved, numerical, conditional,
      open and withdrawn claims separated; records the failed native capacity-to-rate bridge,
      O29/O30 scope, the O31 withdrawal, O32 measurements, and O33 exact finite structure &
      \ssynth \\[3pt]
      ...
      including $g^{\mu\nu}=2\eta^{\mu\nu}$, additionally requires unestablished [H-L] &
      \ssynth \\[3pt]
      ...
      Maps ENI, Foundation, HeisStr version 2.1 and noscale into the audited axiomatic chain:
      ...
      Version 2.0 organises Theorems~A--C of Q14, conditional on a supplied Heisenberg carrier and
      ...
      three-generation factor uses a supplied rank-three carrier. Masses, Yukawa couplings and
      CKM/PMNS mixing remain open.
      Consolidation of the constituent papers; no new result &
      \ssynth \\[3pt]
  ...
  hypothesis [H-F]; Q10 proves $A_H \to 2$ under spectral universality~[U];
  ...
  independent, unestablished spatial-limit hypothesis [H-L]; [H-F] remains separately open.
