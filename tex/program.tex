...
\date{2026\\[2pt]\small Version 2.12}
...
\begin{document}
  ...
  \subsection{Standard Model Companion Papers}
    ...
    The \emph{admissible pair entropy} lives on the rank-3 admissible projection $\Pi_q$: the
    mean-zero lemma of \emph{Q5a-O2}~\cite{Beau2026q5ao2} unconditionally removes the constant
    mode, but spectral atomicity's general analytical proof --- fixing
    $\mathrm{ran}(\Pi_q^{(c)}) = \mathrm{span}\{e_0, e_{\xi_c}, e_{q-\xi_c}\}$ and hence the
    two-dimensional Born--Infeld parity pair $\{e_{\xi_c}, e_{q-\xi_c}\}$ identified with the
    spin-$\tfrac12$ representation --- rests on \emph{O23}'s~\cite{Beau2026a27} own stated
    hypothesis (H-rank): the absence of additional effective symmetries beyond Born--Infeld
    parity, within the scope of \emph{O18}~\cite{Beau2026a22}; the singlet identification of
    \emph{Q3}~\cite{Beau2026q3} then gives $S_{\mathrm{ent}}^{\mathrm{adm}}=\log 2$ for general
    canonical blocks, conditional on both (H-rank) and Q3's explicit, undemonstrated
    diagonal-invariance hypothesis (H-inv) (\sstruct). Q1 does not establish a singlet correlator
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      ...
      admissible projection (mean-zero lemma unconditional via Q5a-O2; atomicity and the singlet
      identification conditional on (H-rank), O23's own stated hypothesis, and Q3's (H-inv), for
