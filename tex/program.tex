...
\date{2026\\[2pt]\small Version 2.5}
...
\begin{document}
  ...
  \begin{abstract}
    ...
    observable states.
    The corpus comprises 115 papers across three theory branches.
    ...
    Branch~III (Q-series and companion papers) develops quantum mechanics, spacetime geometry, and
    gauge dynamics from the Branch~I axioms and supplied Branch~II spectral data: the universal singlet
  ...
  Its carrier-selection step is not derived: the finite countermodel of
  HeisenbergCarrierObstruction~\cite{Beau2026hco} satisfies the algebraic properties available before
  centrality while its commutator is non-central.
  ...
  The corpus comprises 115 papers with complex mutual dependencies.
  The interactive dependency graph covers all 115 entries of the inventory and distinguishes
  ...
  \paragraph{Level 1 --- Axiomatic primitive.}
    ...
    HeisenbergStructure, HeisenbergCarrierObstruction~\cite{Beau2026hco}, noscale~\cite{Beau2026j}.

  \paragraph{Level 2 --- Spectral fibre structure.}
    The O-series supplies the Weil representation $V_\rho$ of
    $\Heis_3(\mathbb{Z}/q\mathbb{Z})$ for a prime~$q$ as its mathematical carrier.
    Foundation and HeisenbergStructure claimed to derive this carrier, but the selection proof is refuted by
  ...
  \subsection{Foundation: Admissible Non-Injective Transitions as the Primitive}
    ...
    The Foundation paper~\cite{Beau2026m} (v1.16) states the axiomatic basis of the programme.
    ...
    The finite \(S_3\) countermodel~\cite{Beau2026hco} has a faithful irreducible unitary carrier, a minimal
  ...
  \subsection{Non-Factorisability and the Emergence of Heisenberg Structure}
    ...
    provides the published proof of Foundation Theorem~5.6, omitted from the
  ...
  \subsection{Finite Countermodel to Heisenberg Carrier Selection}

    HeisenbergCarrierObstruction~\cite{Beau2026hco} gives the six-element countermodel
  ...
  \subsection{Branch~I Synthesis: NIF-Note}
    ...
    HeisenbergStructure~\cite{Beau2026HeisenbergStructure},
    HeisenbergCarrierObstruction~\cite{Beau2026hco}, and noscale~\cite{Beau2026j}
    ...
    \[
      ...
      \;\xrightarrow{\text{postulated carrier}}\;
      V_\rho,
    \]
    ...
    Carrier selection is now the first open structural deliverable.
    NIF-Note also identifies:
  ...
  sub-programme}, whose synthesis paper is the presentation note
    ...
    (Q1 Thm~2.15, conditional on the supplied Weil carrier, from four structural inputs)
    $\Rightarrow$ \emph{Tsirelson bound $|S_{\mathrm{CHSH}}| \leq 2\sqrt{2}$} (Q1 Cor~2.17)
    $\Rightarrow$ \emph{Born rule} (Q1 Thm~2.19, no Gleason theorem)
  ...
  \paragraph{Foundational path.}
    \[
      \mathrm{ENI} \to \mathrm{Found} \to \mathrm{HeisStr}
      \to \mathrm{HCO} \to \mathrm{noscale}.
    \]
    ...
    \begin{figure}[p]
      ...
      \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[
          ...
          \node[S] (Heis)   at (3*\dx,0)      {HeisStr};
          \node[P] (HCO)    at (4*\dx,0)      {HCO};
          \node[P] (nos)    at (5*\dx,0)      {noscale};
...
% aggregates the five constituents of Branch I (ENI, Foundation, HeisStr, HCO, noscale).
          \node[S] (NIFNote) at (2*\dx,\dy) {NIF-Note};
          \foreach \n in {ENI,Found,Heis,HCO,nos}
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
      ...
      Colour coding: \textcolor{cproved}{$\blacksquare$}~\sproved{},
        ...
        its synthesis node: the Branch~I foundational papers (ENI, Foundation, HeisStr, HCO, noscale) into
    ...
    Table~\ref{tab:inventory} lists all 115 papers of the programme, the present registry excepted.
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      \caption{Complete inventory of the 115 programme papers, excluding the present registry.}
      \label{tab:inventory}\\
      \toprule
      ...
      Published proof attempt for Found. Thm~5.6; centrality and prime-order steps fail;
      conditional Stone--von Neumann endpoint remains valid when the carrier data are supplied &
      \sstruct\ /\ \sopen \\[3pt]
      HCO & Heisenberg carrier obstruction & \cite{Beau2026hco} &
      Explicit \(S_3\) countermodel to the carrier-selection implication; direct correction of the
      central-subgroup/Schur step; downstream Heisenberg/Weil mathematics preserved conditionally &
      \sproved \\[3pt]
      ...
      Maps ENI, Foundation, HeisStr, HCO and noscale into the audited axiomatic chain:
      A1--A4 $\Rightarrow$ $\Pi$ non-injective, while the non-trivial commutator does not force
      $\Heis_3(\mathbb{Z}/q\mathbb{Z})$; the Heisenberg/Weil carrier is a supplied realisation.
      Records the white paper's
  ...
  and HeisenbergCarrierObstruction~\cite{Beau2026hco}) is not formally closed.
