...
\begin{document}
  ...
  \subsection{O16--O24: Pair Observable and the Fibre-Level Structure Programme}
    ...
    level of O21 (\sproved{} for the adjoint dimension of the supplied carrier; threshold and
    bridges: \sopen).
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    After W1, the open hypotheses of the former Q5a framework are [H1], [H-E1], [C], and [H2].
    ...
    out the $q^{-1}$ coercivity scale; [H-E1] and [C] are therefore not closed by a finite-rank
    route, and their disposition is the one fixed by Q5a version 3.0: the canonical filtration
    ...
    The framework itself is withdrawn in Q5a version 3.0, and the chain's residual
    conditionality includes the spatial limit hypothesis [H-L] together with the open
    former hypotheses [H1], [H-E1], and [C] in any non-zero-form reading.
    ...
    Identifying the supplied rank-three admissible sector with $\mathrm{Sym}^2(V)$ in the $\mathrm{End}$
  ...
  \subsection{Standard Model Companion Papers}
    ...
    the $\SU(2)$ quaternionic sector (O27 rigidity, conditional on the supplied spinor carrier), and the $\SU(3)$ colour-triplet sector
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{figure}[p]
      ...
      finite-rank route (zero-form limit on the canonical filtration, Q5a version 3.0).
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      ...
      Pre-saturation fingerprints and selected GS vectors are pure Fourier modes; realised
      frequencies are $o(q)$; the three-coordinate reduction is a truncation from which no rank
      is measurable ($\mathrm{HEFF\_DIM}=3$ fixed in advance); the form is $O(q^{-2})$ on
      normalised modes, ruling out the $q^{-1}$ coercivity scale; [H-$\mathcal{E}$1] and [C]
      ...
      after W1, the open hypotheses of the former Q5a framework remain [H1], [H-$\mathcal{E}$1],
      [C] (not closed by Q5a-O2's finite-rank route~\cite{Beau2026q5ao2}), and [H2] &
      \sproved \\[3pt]
      ...
      the supplied saturation rule $\sigma_c(n_3) = 3$ yields the three-generation factor $\mathbb{C}^3_{\mathrm{gen}}$;
      ...
      identification aspect, into the $\mathrm{U}(1)$ phase-fibre, $\SU(2)$ quaternionic (conditional on the supplied spinor carrier) and $\SU(3)$
  ...
  limit on the canonical Fourier-window filtration of Q5a version 3.0.
