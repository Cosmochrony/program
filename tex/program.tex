...
\date{2026\\[2pt]\small Version 2.23}
...
\begin{document}
  ...
  \subsection{Foundation: Admissible Non-Injective Transitions as the Primitive}
    ...
    Q6b, Q8, Q10, Q11, all five now pending revision in the present cascade) to the results table, and a new section placing the
...
\subsection{O1--O8: LPS Phase --- Cascade Exponent \texorpdfstring{$\beta$}{beta}}
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    Q9~\cite{Beau2026q9} proves a kinetic-limit theorem under separately supplied
    kinetic Mosco convergence [K] and compatible recovery [R].
    It does not derive [H-lift] from the frozen, unestablished [H-F].
    ...
    (\sstruct{} for the geometry; conditional on [H-L], and on the identification Q7 version~2.0
    records as supplied by no source, for the metric reading, which is pending revision).
    ...
    $\Pi_q$; its metric reading consumes the unestablished [H-L] and [H-lift].
    Q9 supplies a conditional kinetic-limit criterion, not a discharge of the latter.
    ...
    requiring the identification of the measured admissible space with the spin-$1$ module, supplied
    by no source. Q6b's own remarks call that co-metric fully explicit and fully determined, so
    those conclusions are pending revision.
    ...
    Presents the geometric chain
    $\Pi_q \xrightarrow{\text{Q5a}} \mathcal{L}_\Pi \xrightarrow{\text{Q5b}} g^{\mu\nu}
    \xrightarrow{\text{Gravity}} G_{\mu\nu}$ as closed
    (\scond{} on the Q5a Mosco hypotheses and [H-L]); that closure additionally requires the
    identification, and is pending revision.
    ...
    Non-zero intertwiners exist once the two isomorphic modules are supplied.
    Their geometric compatibility is separate: real rotation invariance [INV] yields horizontal
    isotropy, and the coefficient transport additionally requires an adapted unitary intertwiner
    [ADAPT], not implied by Schur or [ACT].
    Q5b's spatial principal symbol has rank two everywhere and cannot equal a positive rank-three
    Casimir form under invertible transport.
    The revised conjecture instead asks for a new positive spatial target with a specified
    operator and compatible action; lower-order terms cannot change the existing principal rank.
    ...
    The exact Q5b principal-symbol identification is ruled out by rank preservation.
    The broader bridge to a newly supplied positive target remains a missing construction.
    Separately, Q5a supplies a conditional obstruction under its depth and weight hypotheses
    and for common scalar normalisations on toric targets.
    (\sproved{} for the representation theory and scoped rank obstruction;
    \sopen{} for the measured-space identification and for a geometric construction supplying
    a new target with [ACT], [INV] and [ADAPT].)
    ...
    Q9~\cite{Beau2026q9} proves that nonnegative modulation energy leaves a kinetic Mosco limit
    unchanged when the kinetic forms converge under [K] and the same recovery sequences satisfy
    [R]. Its bound $\mathcal V_q(f_q)\leq8\pi^2r_q a_q(X)\|kf_q\|^2/q^2$ retains the energy
    normalisation. The free kinetic form neither selects a nonzero Schr\"odinger character nor
    equals the full oscillator form. A relative moment bound gives an inequality for each
    qualifying vector, not a closed coercive form on a derived linear domain.
    Q5b-O1, [H-lift] and bridge non-obstruction remain open
    (\scond{} on [K] and [R]; version 2.0).
    ...
    U1 version~2.0~\cite{Beau2026u1} withdraws its proof of~[U] and the claimed
    $O(q^{-1/2})$ rate. Its unconditional result is the exact equidistance of the finite
    Heisenberg multiplication generators at distinct central characters. A separate
    adjacent-character example excludes a uniform modulus for their summed operator.
    Neither result proves or refutes~[U], which remains open (\sproved{} for the obstruction;
    \sopen{} for~[U]).
    W1 version~2.0.1~\cite{Beau2026w1} withdraws its claimed proof of~[H-w].
    Its Theorem~4 retains the summation inequality only for a separately defined partial-sum
    proxy under a supplied relative profile estimate; even U1 version~1 did not provide that
    relative estimate.
    Its Lemma~6 identifies a depth-22 target when the cutoff follows the proved five-block
    calibration, while Corollary~7 requires a separate proxy-to-Q5a-response identification.
    Neither the current Q5a directional limits~[H-$w'$] nor the stronger common~[H-w] is proved
    by W1; fibre constancy alone does not give cross-fibre uniformity
    (\scond{} for the proxy bound; \sopen{} for the weight bridge).
    H2's separate result does not close the weight hypotheses; [H-$\mathcal{E}$1] and [C] are
    not closed by the finite-rank route, their disposition being the zero-form one of Q5a version~3.2.
    ...
    \textbf{Q11}~\cite{Beau2026q11}: Claims temporal rigidity under frozen [H-F], but its
    absolute value $A_\tau=2$ additionally imports Q8's scalar normalisation.
    Schur alone does not fix that scalar; the claimed closure of Q5b-O3 is pending revision,
    including its algebraic coefficient value, not only its metric interpretation.
    ...
    had imported back from Q8, Q10, U1 and Q11; U1 is now revised, while the Q8, Q10 and
    Q11 entries await their own stages in this cascade.
    ...
    $g^{\mu\nu} = 2\eta^{\mu\nu}$, whose coefficient values are pending revision, supports an
    Einstein sector: from the projective spectral
    ...
    carrier and central character, the unestablished Lorentzian-geometry hypothesis [H-L] whose
    coefficient values are pending revision,
    ...
    so the existence and stability of the split are governed by the Lorentzian genus of
    the chiral polarisation $P_\chi$ in $g^{\mu\nu} = 2\eta^{\mu\nu}$, whose coefficient values are
    pending revision. The identification of the
  ...
  \subsection{Lorentz Capacity Sub-Programme}
    ...
    $g^{\mu\nu} = 2\eta^{\mu\nu}$, whose asserted coefficient values are pending revision in the
    present cascade. A guiding distinction runs throughout: the Born--Infeld capacity
    ...
    The chain assembled by the sub-programme is
    \[
      \text{BI budget} \to F^\tau = 1/\gamma \to g^{\mu\nu} = 2\eta^{\mu\nu} \to \Lambda(\beta)
      \to R(x)^2 = 1-\epsilon(x) \to \text{finite-}q\text{ domain,}
    \]
    with the co-metric step carrying the coefficient values now pending revision.
    ...
    Lorentzian splitting of the Q5b co-metric $g^{\mu\nu} = 2\eta^{\mu\nu}$, whose asserted
    coefficient values are pending revision in the present cascade,
    ...
    With the Q11 identification $g^{\tau\tau} = -A_{\tau}$ (flat-space value $-2$, pending
    revision) and a diagonal
    ...
    the effective co-metric $g^{\mu\nu} = 2\eta^{\mu\nu}$, whose coefficient values are pending
    revision, is the standard Lorentz boost;
    ...
    \emph{inertial} kinematics, up to the co-metric coefficient values it takes from Q5b--Q11,
    which are pending revision.
...
% aggregates Q1, Q2, Q3, and the Bell paper.
          ...
          \node[font=\tiny\sffamily,text=black!55,above=1pt] at (Q7.north)
            {identification open};
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{figure}[p]
      ...
      \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[
          ...
          \node[font=\tiny\sffamily,text=black!55,above=1pt] at (Q7.north)
            {identification open};
          \node[S] (Q8)  at (2*\dx,-3*\dy){Q8\\{\tiny $A_Z\!=\!2$ under [H-F]; pending}};
          \node[C] (Q9)  at (\dx,-3*\dy) {Q9\\{\tiny kinetic under [K], [R]}};
          \node[C] (Q10) at (\dx,-4*\dy) {Q10\\{\tiny $A_H\!\to\!2$ under [U]; pending}};
          \node[P] (U1)  at (\dx,-5*\dy) {U1\\{\tiny obstruction proved; [U] open}};
          \node[C] (W1)  at (2*\dx,-5*\dy){W1\\{\tiny proxy; [H-w] open}};

          % Q11 and final result
          \node[S] (Q11) at (\dx+0.5*\dx,-6*\dy)
            {Q11\\{\tiny temporal scale from Q8; pending}};
          ...
          \draw[arr,dashed] (Q5aO2) to[out=-90,in=90] (W1);
          \draw[arr]   (Q5b) to[out=0,in=135] (Q7);
          \draw[arr]   (Q5b) to[out=-45,in=90] (Q9);
          \draw[arr,dashed] (Q5b) to[out=-90,in=90] (U1);
          ...
          \draw[arr,dashed] (Q10)--(U1);
          \draw[arr,dashed] (U1)--(W1);
          \draw[arr,dashed] (Q5a) to[out=-45,in=90] (W1);

          \draw[arr,dashed] (U1) to[out=-90,in=135] (Q11);
          \draw[arr]   (Q8)  to[out=-90,in=45]  (Q11);
          \draw[arr,dashed] (W1)  to[out=-90,in=45]  (Q11);
          \draw[arr]   (Q11) -- (Met);

          % ── Status annotations ───────────────────────────────
          \node[font=\tiny\sffamily,text=black!55,right=2pt] at (Q5aO2.east)
            {pure modes; truncation status};
          \node[font=\tiny\sffamily,text=black!55,left=12pt] at (Q9.west)
            {lift open};
          \node[font=\tiny\sffamily,text=black!55,right=2pt] at (W1.east)
            {Q5a weight bridge open};

        \end{tikzpicture}
      }
      \caption{Geometric emergence sub-programme: dependency map from Q5a to the proposed effective
      ...
      \textbf{Q9}~(\scond{} on [K] and [R]): a nonnegative modulation term leaves a supplied
        kinetic Mosco limit unchanged when each finite-energy vector has one kinetic recovery
        sequence on which the modulation energy vanishes. The bound retains its energy scale;
        the free limit does not select a Schr\"odinger character or close [H-lift].
        ...
        \textbf{U1}~(\sproved{} for an unconditional obstruction; \sopen{} for [U]):
        exact equidistance of distinct-character Heisenberg multiplication generators;
        a separate adjacent-character example obstructs the claimed summed-operator modulus.
        The former [U] proof and $O(q^{-1/2})$ rate are withdrawn.
        \textbf{W1}~(\scond{} for its proxy bound; \sopen{} for [H-w]): a supplied
        relative profile estimate bounds a separately defined finite-window sum.
        The former proof of convergence of Q5a's response-average weights is withdrawn;
        fibre constancy alone does not close their identification.
        ...
        version~2.0 records as supplied by no source: Q7 is \sproved{} for the requirements it states on the
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      ...
      Q7 version~2.0 audit, under the identification [ID] that no source supplies; that reading is
      pending revision.
      Spatial limit input formalised as the unestablished hypothesis [H-L], on a supplied Heisenberg
      carrier (version 2.1); Q9 gives a conditional kinetic limit under [K] and [R], without
      discharging [H-lift]; metric results remain conditional on [H-L] and [H-lift];
      Carnot/$D_{\mathrm{hom}}=4$ geometry independent &
      \scond{} on [H-L] and [H-lift] (metric chain); \sstruct{} (geometry) \\[3pt]
      ...
      Q9 supplies only a kinetic limit under [K] and [R]; metric claims
      conditional on the independent [H-L] and [H-lift] and on the identification Q7 version~2.0 records as
      supplied by no source. Q6b's own remarks call the co-metric ``fully explicit'' and ``fully
      determined'' on the strength of Q8, Q10 and Q11, so those conclusions are pending revision &
      \scond{} on the Q5a Mosco hypotheses ([H1], [H-w], [H-$\mathcal{E}$1], Conjecture~C), [H-L], [H-lift] and
      that identification \\[3pt]
      ...
      source. The exact Q5b principal-symbol target is excluded by rank; a new compatible
      positive target remains open &
      \sproved{} (requirements on the supplied carrier); \sopen{} (the identification, and the
      new target under [ACT], [INV] and [ADAPT]) \\[3pt]
      ...
      version~2.0 records as supplied by no source. Q7 records the support for these values as
      circular: the scalar was fixed by matching a value itself obtained by inserting $A_Z$.
      Pending revision &
      \sstruct\ /\ \sopen \\[3pt]
      Q9 & Kinetic limit under [K] and [R] & \cite{Beau2026q9} &
      A nonnegative modulation term leaves a supplied kinetic Mosco limit unchanged when
      compatible recovery sequences suppress it. The bound is
      $\mathcal V_q(f_q)\le8\pi^2r_q a_q(X)\|kf_q\|^2/q^2$.
      The free form selects no nonzero central character; a relative moment bound gives only an
      elementwise inequality on a cone. Q5b-O1, [H-lift] and bridge non-obstruction remain open
      (version 2.0) &
      \scond{} on [K] and [R] \\[3pt]
      ...
      (as are the Q5b, Q6b, Q8 and Q11 rows; U1 and W1 are revised below) &
      \scond{} on [U] \\[3pt]
      U1 & Obstruction to the generator route to [U] & \cite{Beau2026u1} &
      Version~2.0 proves the exact equidistance of finite Heisenberg multiplication
      generators at distinct characters.
      A separate example excludes the claimed modulus for their sum.
      It withdraws the former proof of [U], its
      $O(q^{-1/2})$ rate, the small-$\theta$ conclusion and the closure of Q10 and Q7.
      The O25 observable is a sampled-block rank increment, not an identified
      compressed operator norm.
      [U] remains open &
      \sproved{} / \sopen \\[3pt]
      W1 & Finite-window proxy; [H-w] open & \cite{Beau2026w1} &
      Version~2.0.1 withdraws the claimed common positive limit of Q5a's
      fingerprint-response weights.
      Theorem~4 proves only a summation inequality for a separate proxy under a supplied
      relative profile bound, never proved by U1.
      Lemma~6 gives a depth-22 target under the specified five-block calibration,
      not an infinite-series coefficient; Corollary~7 exposes the missing
      proxy-to-weight identification.
      Q5a's directional~[H-$w'$] and the stronger common~[H-w] remain open.
      H2~\cite{Beau2026h2} does not discharge them &
      \scond{} / \sopen \\[3pt]
      ...
      Claims $A_\tau=C_{\mathfrak{su}(2)}=2$ under frozen [H-F], but fixes the scalar by
      importing Q8's normalisation; the absolute temporal value therefore also requires revision;
      the co-metric reading $g^{\mu\nu}=\mathrm{diag}(-2,2,2,2)\propto\eta^{\mu\nu}$
      additionally requires the independent, unestablished [H-L] and the identification Q7
      version~2.0 records as supplied by no source. Pending revision &
      \sstruct\ /\ \sopen \\[3pt]
      ...
      On a supplied Heisenberg carrier and a conditional [H-L] geometry whose coefficient values are
      pending revision, the spinor bundle $S_\Pi$
      ...
      whose coefficient values are pending revision;
      ...
      Q8 consumes frozen, unestablished [H-F]; Q9 gives a conditional kinetic-limit criterion
      under [K] and [R] but does not discharge [H-lift]. Every effective co-metric or Lorentzian reading,
      including $g^{\mu\nu}=2\eta^{\mu\nu}$, additionally requires unestablished [H-L], [H-lift] and the
      ...
      $g^{\mu\nu} = 2\eta^{\mu\nu}$, whose coefficient values are pending revision, supports an
      Einstein sector: $a_2$ infrared response of the
      ...
      Lorentz group as isometry group of the effective metric; the co-metric coefficient values are
      pending revision.
      Consolidation of the constituent papers; no new result &
      \ssynth \\[3pt]
    \end{longtable}
  ...
  \paragraph{Bridge existence at finite $q$ (Q7--Q10).}
    Q10 still asserts $A_H \to 2 = A_Z$ conditionally on~[U], which U1 version~2.0
    leaves open rather than proving.
    Q7 version~2.0 no longer reads that as establishing the isotropic limit of its
    former bridge claim; the revised Conjecture~4.7 concerns a new positive target, and it withdraws its own use
    of those results; Q10 remains pending revision in this cascade.
    The exact Q5b rank-two principal symbol cannot realise the positive rank-three target.
    The revised Conjecture~4.7 asks instead for a new spatial construction supplying the target,
    action and compatible adapted transport.
    Abstract intertwiner existence is already guaranteed for the supplied isomorphic modules.
  ...
  \paragraph{Lorentz Capacity Sub-Programme.}
    The Lorentz-capacity sub-programme is closed for inertial kinematics, up to the co-metric
    coefficient values it takes from Q5b--Q11, which are pending revision; it is open in its
  ...
  The geometric emergence sub-programme has advanced substantially: Q9 gives a kinetic Mosco
  limit under [K] and [R], with no derived [H-lift] or $A_H>0$ coefficient;
  Q10 asserts $A_H \to 2$ under spectral universality~[U];
  U1 version~2.0 withdraws its proof of [U] and its $O(q^{-1/2})$ rate; it proves an
  unconditional equidistance obstruction for finite Heisenberg generators and leaves
  [U] open. W1 version~2.0.1 withdraws its claimed proof of~[H-w]: its
  conditional sum bounds a proxy, while the identification with Q5a's
  response-average weights and their positive limits remain open.
  The effective Lorentzian co-metric $g^{\mu\nu} = \mathrm{diag}(-2, 2, 2, 2) \propto \eta^{\mu\nu}$
  is claimed by Q11~\cite{Beau2026q11}, whose absolute temporal value $A_\tau=2$ imports Q8's
  scalar normalisation and is itself pending revision.
  Its proposed metric interpretation additionally requires the
  independent, unestablished spatial-limit hypothesis [H-L] and the identification of the
  ...
  The Q5b, Q6b, Q8, Q10 and Q11 entries are pending revision in the cascade that
  Q7 version~2.0 opens; the marker is carried on the surfaces that assert the affected results.
  ...
  Q5a, Q5b, Q7--Q11, U1, W1 and H2, with Q9's kinetic result independent of Q7 and Q8
  and with the geometric and coefficient steps still open.
