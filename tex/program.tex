...
\date{2026\\[2pt]\small Version 1.97}
...
\begin{document}
  ...
  \begin{abstract}
    ...
    observable states.
    The corpus comprises 112 papers across three theory branches.
    ...
    Branch~II (O-series) develops the spectral admissibility sub-programme, derives the
    conjugate-pair fibre structure and measures its capacity exponent $\delta_{\mathrm{pair}}$.
    ...
    The same branch identifies the Standard Model gauge group
  ...
  O24~\cite{Beau2026a28} closes the conjugate-pair construction with respect to the fibre structure
  of $\Pi$.
  ...
  The corpus comprises 112 papers with complex mutual dependencies.
  The interactive dependency graph covers all 112 entries of the inventory and distinguishes
  ...
  \paragraph{Conventions on result status.}
    ...
    A fifth entry, \ssynth{}, is reserved in the inventory table for the sub-programme presentation
  ...
  \paragraph{Level 2 --- Spectral fibre structure.}
    ...
    The O-series computes the spectral properties of this fibre, including the capacity exponent
    $\delta_{\mathrm{pair}}$.
  ...
  \begin{equation}
    ...
    \label{eq:chain}
  \end{equation}
  The first segment is established with respect to the fibre structure of $\Pi$
  (O24~\cite{Beau2026a28}).
  ...
  O1--O33 --- into a single account of the proposed chain~\eqref{eq:chain}, recording the status of
  ...
  \subsection{O9--O15: Heisenberg Transition}
    ...
    The exact native identity is $\Delta r_n=|S_n|\Sigma_n$; no reweighting of block data
    supplies the missing LPS growth carrier.  The former reciprocal outputs are retained only
    as phenomenological diagnostics (\sproved{} for the stated aggregation and native-identity
    results).

  \subsection{O16--O24: Pair Observable and Closure of the Fibre-Level Structure}
    ...
    $\Sigma_c(n_3) = 3$ and formulates the conditional conversion
    $\delta_{\mathrm{pair}} \to \beta^*$ using the O4--O7 inputs.
    SGN proves that this conversion has no native Heisenberg carrier (\sstruct{} as a
    cross-substrate prescription; native derivation: refuted for the corpus-defined objects).
    ...
    (\sproved{} under the Weil realisation and stated fibre hypotheses).
  ...
  \subsection{O25--O30, PTO, and Fibre Erasure: Numerical Validation, Sector Identification, Eigenvalue Ratio, and Information-Theoretic Axes}
    ...
    equivariance, quadratic compatibility, and naturality factors through $\mathfrak{su}(2)$
    (\sproved).  The theorem gives $\delta_{\mathrm{pair}}$ a representation carrier, not a
    derived interpretation of $\beta^*$.
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    Table~\ref{tab:inventory} lists all 112 papers of the programme, the present registry excepted.
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      \caption{Complete inventory of the 112 programme papers, excluding the present registry.}
      \label{tab:inventory}\\
      \toprule
      ...
      Conditional block-normalisation correction; SGN recovers the shell factor additively but
      leaves the inter-$q$ estimator layer open &
      \sstruct \\[3pt]
      ...
      conditional cross-substrate prescription $\delta_{\mathrm{pair}} \to \beta^*$ &
      \sstruct \\[3pt]
      ...
      Verticality Lemma closes $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}}$ w.r.t.\ the fibre
      structure of $\Pi$; no native derivation of $\delta_{\mathrm{pair}} \to \beta^*$ &
      \sproved \\[3pt]
      \midrule
      O25 & Numerical campaign (extended to $q=601$) & \cite{Beau2026a29} &
      Raw $\delta_{\mathrm{global}} \to 7.61$ at $q=601$ under the frozen $\log(n+1)$ estimator;
      fixed-$q$ pair concentration; O14 correction overcorrects at large $q$;
      ...
      connects $\Heis_3$ to $2I \subset \SU(2)$; no rate equation follows &
      \sstruct \\[3pt]
      ...
      $\mathfrak{su}(2)$; representation carrier for $\delta_{\mathrm{pair}}$, not $\beta^*$ &
      \sproved \\[3pt]
      ...
      the raw frozen-estimator exponent reaches the historical window while the O14 endpoint
      correction overcorrects at large $q$; $1\%$ threshold rank
      ...
      finite-window law; corpus-native [T1], pair-carrier, and exponent-coordinate obstructions
      ...
      corrects Q7 ($e_0$ is the weight $m=0$ of the $j=1$ module, not a $j=0$ state) &
      \sproved\ /\ \sopen \\[3pt]
      \midrule
      ...
      $S_{\mathrm{ent}}(n)=\log r_{\mathrm{pair}}(n)$ is derived analytically
      (literal Schmidt pairing for independently sampled canonical O25 blocks: open) &
      \sproved\ /\ \sopen \\[3pt]
      ENT-E2 & Two entanglement entropies of the conjugate pair & \cite{Beau2026ent2} &
      Separates the admissible pair entropy $S_{\mathrm{ent}}^{\mathrm{adm}} = \log 2$ on the rank-3
      admissible projection (unconditional for general canonical blocks via O21 atomicity, mean-zero
      lemma and the Q3 singlet) from the full residual-rank entropy
      ...
      $[\mathrm{H}_{\mathrm{res}}]$ for general blocks); excludes the metaplectic dilation as a bridge
      for $[\mathrm{H}_{\mathrm{res}}]$ &
      \sproved\ /\ \sstruct \\[3pt]
      ...
      Maps the five precursor papers and O1--O33 into a single account of the spectral admissibility
      sub-programme, recording the status of every result as proved, structural, numerical, or open.
      ...
      Maps the twelve constituents (Q5a, Q5a-O2, Q5b, Q6b, Q7--Q11, U1, W1, H2) into the chain
      $\Pi_q \to L_{\mathrm{eff}} \to g^{\mu\nu} = 2\eta^{\mu\nu}$, from the admissibility filter to the
      effective metric. Consolidation of the constituent papers; no new result &
      \ssynth \\[3pt]
      GS\-Note & The gauge structure sub-programme (Note~3) & \cite{Beau2026gsnote} &
      Organises Foundation, TopInv, SMcm, O23, O27--O32, Q6a, Q7 and Q12, in their gauge-group
      identification aspect, into the $\mathrm{U}(1)$ phase-fibre, $\SU(2)$ quaternionic and $\SU(3)$
      colour-triplet sectors of $G_\Pi$. Gauge group identification only; Yang--Mills dynamics belongs to
      ...
      no new result &
      \ssynth \\[3pt]
      NIF\-Note & The non-injective foundations sub-programme (Note~5) & \cite{Beau2026ninote} &
      Maps ENI, Foundation, HeisStr and noscale into the axiomatic chain
      A1--A4 $\Rightarrow$ $\Pi$ non-injective $\Rightarrow$ $[X,\sigma(X)] = Z \neq 0$ $\Rightarrow$
      $\Heis_3(\mathbb{Z}/q\mathbb{Z})$ $\Rightarrow$ $V_\rho$, recording the white paper's
      $\chi$/relaxation vocabulary as superseded by the axiomatic formulation.
      Consolidation of the constituent papers; no new result &
      \ssynth \\[3pt]
      FM\-Note & The fermionic matter sub-programme (Note~6) & \cite{Beau2026fmnote} &
      Organises Theorems~A--C of Q14 into the chain from $\Pi$ to the spinorial face of the admissible
      Weil module: $\Sym^2(\mathcal{S}_\Pi) \simeq \mathrm{ad}_\mathbb{C}(P_\Pi)$ and
      $\wedge^2(\mathcal{S}_\Pi) = L_Y$ reproducing $\SU(2)_L \times \mathrm{U}(1)_Y$, $V{-}A$ chirality
      and hypercharge rigidity, and the three-generation factor $\mathbb{C}^3_{\mathrm{gen}}$.
      Consolidation of the constituent papers; no new result &
      \ssynth \\[3pt]
      QS\-Note & The quantum structure sub-programme (Note~7) & \cite{Beau2026qsnote} &
      Maps Q1, Q2, Q3 and Bell into a six-step chain from BI indiscernibility to the universal spin-$j$
      correlator, isolating the upstream inputs consumed and the two open deliverables (Born rule beyond
      $\SU(2)$; numerical validation of the coherence witness across the prime range).
      Consolidation of the constituent papers; no new result &
      \ssynth \\[3pt]
      GGS\-Note & The gauge--gravity stratification sub-programme (Note~8) & \cite{Beau2026ggsnote} &
      Organises Q12 and Q13 around the spectral stratification principle
      ...
      hierarchy claimed. Consolidation of the constituent papers; no new result &
      \ssynth \\[3pt]
      ...
      monotonicity along the cascade; v1.1 integrates the ENT-E1 and ENT-E2 results and records the
      residual epistemic stratification. Consolidation of the constituent papers; no new result &
      \ssynth \\[3pt]
  ...
  \paragraph{$V-A$ chiral structure and fermionic matter (Q14).}
    ...
    $\varepsilon = \mathcal{N}_{\mathrm{casc}}|\alpha|$; the former
  ...
  Branch~II (O-series) has closed the conjugate-pair fibre structure through
  $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}}$~(O24) and measured the pair exponent through
  $q=601$~(O25).
