...
\begin{document}
  ...
  \begin{abstract}
    ...
    The corpus comprises 110 papers across three theory branches.
    ...
    validating the transfer chain $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^* \approx 0.126$
    ...
    O33~\cite{Beau2026a37} gives an exact arithmetic description of the O12 three-character
    filtration, derives its Mackey--Hecke multiplicity doublets and Harper split, and proves a
    parity-closure obstruction protecting the odd Weyl-symbol sector.
    Its stabiliser analysis now includes an orbit-divisibility sieve, uniform exact
    $C_4$ and $C_6$ exceptional families, and (v1.1) an additive rigidity theorem proving
    $A_K=\{\pm1\}$ whenever every run and gap of the mode set is longer than the run count,
    together with a third uniform near-saturation $C_4$ family; completeness of the
    exceptional orders remains open only in the residual degenerate boundary regime.
    Version 1.4 describes that regime mechanically --- the depth-zero square extends to an
    infinite uniform Gaussian-box $C_4$ family, and an exhaustive audit through $q=151$
    classifies all $75$ exceptional sets into three refined mechanisms with no order beyond
    six --- and proves that the porous depths of a uniform block converge to an explicit
    limiting point process with rational count law and mean $28/15$.
    Version 1.5 fixes the exact stabiliser value inside two of those mechanisms: a
    fourth-moment argument proves that every proper Gaussian box, direct or wrapped, has
    stabiliser exactly $C_4$, and every free four-point complement orbit has the same exact
    stabiliser; completeness of the three refined mechanisms, and a uniform identification of
    the top autocorrelation level of the Eisenstein $C_6$ sets, remain open.
    Its mixing criterion is closed (v1.3): a Galois separation theorem removes the cyclotomic
    phase determinant for every generic character, and the boundary-sum zeros are completely
    classified into antipodal-pair, cube-triple, and double-pair mechanisms on at most four
    hole pairs.
  ...
  The corpus comprises 109 papers with complex mutual dependencies.
  ...
  \subsection{Branch~I Synthesis: NIF-Note}
    ...
    of noscale (every emergent scale as a functional of~$c_{\mathrm{BI}}$ alone).
  ...
  \begin{equation}
    c_{\mathrm{BI}} \;\longrightarrow\; \delta_{\mathrm{pair}} \;\longrightarrow\; \beta^* \approx 0.126
  ...
  O1--O33 --- into a single account of the derivation chain~\eqref{eq:chain}, recording the status of
  ...
  \subsection{O16--O24: Pair Observable and Unconditional Transfer Chain}
    ...
    unconditional transfer chain $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^*$
  ...
  [H-color] proved analytically via O31 v1.5).
    ...
    three-character filtration (exact core).
    ...
    sum $C_\theta(K)$ is non-zero (v1.3): a Galois separation theorem proves
    ...
    In v1.4 the degenerate regime is described mechanically: the depth-zero square extends
    to an infinite uniform Gaussian-box family $Q_r(i)$ with $C_4\subseteq A_K$, realised
    at depths $r-1$ for $r\le4$, so the earlier exhaustion reading of the family statements
    is withdrawn; an exhaustive audit through $q=151$ classifies all $75$ exceptional sets
    ...
    conjectural.
    Version 1.5 proves the exact stabiliser value inside two of the three mechanisms.
    ...
    Version 1.4 also proves the asymptotic porosity law: the porous depths of a uniform
  ...
  \sim (12\cdot 16\pi^2)^{-1}\log(\Lambda/\mu)$,
    ...
    same $c_{\mathrm{BI}}$ saturation bound, recovering both sector-specific BI completions as limits
    ...
    base manifold; BornInfeld for the saturation bound $c_{\mathrm{BI}}$ and the conditionality
    ...
    $\SU(2)_L \times U(1)_Y$ gauge-sector bundle structure (\sstruct); these are
    Lorentz-typed adjoint and determinant data, explicitly distinguished from the
    independent fermionic weak doublet, which is not produced at this
    stratum~\cite{Beau2026nsr}.
  ...
  \subsection{Gravity and Spacetime Track}
    ...
    A subsequent notational and structural clarification separates the global matter--geometry multiplier
    $\vartheta_*^{-1}$ from the local spectral field $\beta(x)^{-1}$ and from the cascade exponent $\beta^*$.
    In the gauge sector, $\vartheta_*^{-1} = 1$ is identified conditionally as the single-determinant value
    fixed by the relative $a_2/a_4$ normalisation of the joint spectral functional~\cite{Beau2026q13}.
    The off-equilibrium term renormalises the coupling to the same matter content rather than defining an
    independent fluid, while the $a_2$ channel is classically silent for traceless radiation (\sstruct).
  ...
  \subsection{Cosmological and Condensed-Matter Applications}
    ...
    corrections or the spectral-equilibrium route~\cite{Beau2026Thermodynamics}); it also records the
    ...
    (mechanism and no-fit grid: \sstruct/numerical; entropy-to-power step: conditional theorem chain in
    ProjCE~\cite{Beau2026pce}; TE/EE transfer: \sopen).
    ...
    A quadrupole-anchored, parameter-free $2\times2$ grid of envelopes and angular dictionaries improves the
    Planck TT low-$\ell$ diagnostic $\chi^2$ from $27.4$ to $17.3$ in the theoretically preferred cell
    (derived Bessel envelope with the H-dictionary), with TE neutral and EE mildly disfavoured; the envelope
    is a staircase in $\ell$ (integer ranks), a discrete signature.
    ...
    \begin{figure}[p]
      ...
      \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[
          ...
          \node[font=\tiny\sffamily,text=cproved,right=2pt] at (O33.east)
            {Weil doublets};
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{longtable}{@{}p{1.45cm}p{3.4cm}p{1.5cm}p{6.5cm}p{1.3cm}@{}}
      ...
      $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^* \approx 0.126$ (Cor.~4.2) &
      \sproved \\[3pt]
      \midrule
      ...
      its zeros completely classified on at most four hole pairs (v1.3);
      degenerate regime refined (v1.4): infinite Gaussian-box $C_4$ family, all 75
      exceptional sets through $q=151$ in three mechanisms, no $C_8$ through $q=257$;
      porous depths converge to an explicit point process with mean $28/15$ (v1.4);
      ...
      completeness and the Eisenstein top level open (v1.5);
      ...
      Admissible spinor bundle $S_\Pi$ reproduces the $\SU(2)_L \times U(1)_Y$ gauge-sector
      ...
      Parameter-free small-$N$ replacement of the phenomenological low-$\ell$ envelope: no-fit
      $2\times2$ grid improves Planck TT diagnostic $\chi^2$ $27.4\to17.3$ (preferred cell), TE neutral,
      EE mildly disfavoured (open transfer); staircase envelope; $8/3$ witness re-audited with observable
      $\sqrt{8/3}$ &
      \sstruct \\[3pt]
  ...
  \paragraph{$V-A$ chiral structure and fermionic matter (Q14).}
    ...
    $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^*$), the explicit mass generation mechanism,
  ...
  Branch~II (O-series) has closed the unconditional transfer chain $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^*$
