...
\date{2026\\[2pt]\small Version 2.22}
...
\subsection{O1--O8: LPS Phase --- Cascade Exponent \texorpdfstring{$\beta$}{beta}}
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    observables ($\mathcal{O} = \mathrm{Im}\,\Pi$); its quantitative step imports the
    rate now withdrawn by U1 and awaits W1's own revision (\scond{} on [H1]; pending).
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{figure}[p]
      ...
      \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[
          ...
          \node[C] (W1)  at (2*\dx,-5*\dy){W1\\{\tiny [H-w] pending audit}};
      ...
      \textbf{Q9}~(\scond{} on [K] and [R]): a nonnegative modulation term leaves a supplied
        ...
        ($\mathcal{O}=\mathrm{Im}\,\Pi$); quantitative control formerly imported from U1
        is withdrawn, leaving W1 pending its own audit.
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      ...
      (as are the Q5b, Q6b, Q8, W1 and Q11 rows; U1 is revised below) &
      \scond{} on [U] \\[3pt]
      ...
      ($\mathcal{O}=\mathrm{Im}\,\Pi$) is the proposed structural origin; the U1 rate
      imported for quantitative control is withdrawn, so W1's result awaits audit.
  ...
  [U] open. W1's claim of [H-w] imports the withdrawn U1 rate and awaits its own audit.
  ...
  The Q5b, Q6b, Q8, Q10, W1 and Q11 entries are pending revision in the cascade that
