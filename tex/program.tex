...
\begin{document}
  ...
  \subsection{SpectralRelaxation: Expander Graph Dynamics}
    ...
    for LPS expander families (\scond{} on the capacity--isoperimetry closure hypothesis), providing the
...
\subsection{O1--O8: LPS Phase --- Cascade Exponent \texorpdfstring{$\beta$}{beta}}
    ...
    that the paper's own fingerprint does not satisfy (\sproved\ /\ \scond{} on the power-law Ansatz).
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    and the unique such form is the Casimir with value~2 (\scond{} on [U]).
    ...
    After W1 and H2, the open hypothesis of the former Q5a framework is [H1]; [H-E1] and [C] are not
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      ...
      \sproved\ /\ \scond{} on the power-law Ansatz \\[3pt]
      ...
      of O16~\cite{Beau2026a20} forcing $r(c,q)\equiv1$ only on its own model &
      \sopen\ /\ \sproved \\[3pt]
      ...
      within the stated model, on two sketch-level premises (the continuous decay law and the shell
