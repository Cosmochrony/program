...
\begin{document}
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    (\sstruct; the coefficient identification $8\pi G_N = c_{\mathrm{YM}}/c_{\mathrm{EH}}$ is
    pending the Q13 re-audit).
    ...
    The difference in ultraviolet divergence degree between the two sectors is structural
    ($a_2 \to$ gravity at quadratic UV, $a_4 \to$ gauge at logarithmic UV).
    The numerical hierarchy $G_N g_{\mathrm{YM}}^2 \ll 1$ is, however, \emph{not} a tuning-free
    output: $g_{\mathrm{YM}}$ and $G_N$ are renormalized matching data, so the ratio is fixed by
    their finite matching values, and the Q13 estimate
    $G_N g_{\mathrm{YM}}^2 \sim (\ell_{\mathrm{sp}}^2/\dim V)\,[\log(\Lambda/\mu)]^{-1}$ is
    pending re-audit.
    ...
    between the sectors ($a_2$ quadratic vs.\ $a_4$ logarithmic).
    Q13 supplies the coupled Einstein--Yang--Mills system, the $a_6$ cross-coupling, and the
    Eddington--Born--Infeld joint completion (conditional on [H-ext]); its hierarchy claim
    $G_N g_{\mathrm{YM}}^2 \ll 1$ is pending re-audit and is not presented here as established,
    since $g_{\mathrm{YM}}$ and $G_N$ are renormalized matching data.
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{longtable}{@{}p{1.45cm}p{3.4cm}p{1.5cm}p{6.5cm}p{1.3cm}@{}}
      ...
      divergence degree, no tuning-free hierarchy claimed
      (\sproved{} for the $a_4$ derivation; conditional on Q6a for $G_\Pi$) &
      \sstruct \\[3pt]
      ...
      hierarchy ratio $G_N g_{\mathrm{YM}}^2$ pending re-audit ($g_{\mathrm{YM}}$, $G_N$ are
      matching data) &
      \sstruct \\[3pt]
  ...
  \paragraph{Gauge--gravity synthesis (Q12 $\to$ Q13).}
    Q12 establishes Einstein gravity ($a_2$) and Yang--Mills dynamics ($a_4$) as sectors of the
    single functional $S_\Pi[g,\mathcal{A}]$: at fixed base metric the vertical variation
    ...
    $T^{\mathrm{YM}}_{\mu\nu}$.
    Q13~\cite{Beau2026q13} aims to complete this synthesis (joint variational problem
    $\delta_{g,\mathcal{A}} S_\Pi = 0$, coupled Einstein--Yang--Mills system, and the
    Eddington--Born--Infeld completion); its hierarchy ratio $G_N g_{\mathrm{YM}}^2$ is pending
    re-audit and is not presented here as an established prediction, since $g_{\mathrm{YM}}$ and
    $G_N$ are renormalized matching data.
  ...
  matching data.
  Q13~\cite{Beau2026q13} aims to complete the gauge--gravity synthesis (joint variational
  problem $\delta_{g,\mathcal{A}} S_\Pi = 0$, coupled Einstein--Yang--Mills system). What is
  structural is the difference in UV divergence degree between the sectors ($a_2$ quadratic
  vs.\ $a_4$ logarithmic); the numerical hierarchy $G_N g_{\mathrm{YM}}^2 \ll 1$ is not a
  tuning-free prediction and is pending re-audit of Q13.
