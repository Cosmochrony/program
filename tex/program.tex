...
\begin{document}
  ...
  of the substrate, culminating in the proof in O24~\cite{Beau2026a28} of the transfer chain
  $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^*$, unconditional with respect to the fibre
  structure of $\Pi$ (conditional on the O4--O6 growth-equation closure and coupling hypotheses,
  the Weil realisation, and the power-law regime).
  ...
  \paragraph{Level 3 --- Physical observables.}
    ...
    [H-L] of Q5a--Q5b (unestablished; Q5a version 3.0, Q5b version 2.0).
  ...
  \subsection{Foundation: Admissible Non-Injective Transitions as the Primitive}
    ...
    \sopen{} for the continuum limit (Q5a version 3.0 withdraws the Mosco derivation; the
    spatial limit is the unestablished hypothesis [H-L] of Q5b version 2.0).
  ...
  \subsection{O16--O24: Pair Observable and Closure of the Fibre-Level Transfer Chain}
    ...
    transfer chain $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^*$ unconditionally with
    respect to the fibre structure of $\Pi$
    (Corollary~4.2: $\beta^* \approx 0.126$, \sproved; the O4--O6 growth-equation closure and
    coupling hypotheses, the Weil realisation, and the power-law regime remain its standing
    conditions, and the numerical value remains open pending $\delta_\infty$).
  ...
  \subsection{O25--O30, PTO, and Fibre Erasure: Numerical Validation, Sector Identification, Eigenvalue Ratio, and Information-Theoretic Axes}
    ...
    The covariance $\mathcal{C}_c$ has $1\%$ threshold rank $r_{\mathrm{eff}}^{1\%} = 3$ for every
    conjugate pair and every $q \in \{61, 101, 151\}$ (100\% of pairs), with invariant resolved
    spectrum $[\lambda_1 : \lambda_2 : \lambda_3] = [1 : \tfrac{1}{2} : \tfrac{1}{2}]$.
    The exact degeneracy $\lambda_2 = \lambda_3$ is read, through O29, as the axial structure of
    the adjoint carrier (interpretation), correlated with the Born--Infeld parity involution
    $c \leftrightarrow q-c$ (O18).
    ...
    \textbf{Span-Growth Note}~\cite{Beau2026sgn}: Substrate-level classification of the cumulative
    fingerprint span on the Heisenberg cascade.
    The exact increment identity $\Delta r = \sigma_c\,|S_n|$ and the exact polynomial spheres
    ($D_{\mathrm{hom}} = 4$, Bass--Guivarc'h) force a three-branch law depending only on
    $(\delta_c, D)$: growth $r \sim n^{D-\delta_c}$ for $\delta_c < D$, logarithmic growth at
    $\delta_c = D$, saturation $r_\infty - r \sim n^{-(\delta_c - D)}$ strictly for
    $\delta_c > D$ (\sproved).
    Two analytic corollaries: $r/|B_n| \to 0$ for every $\delta_c > 0$ (no valence--exploration
    proportionality on this substrate), and no uniform constant sustains
    $\Delta r \le c\,\sigma_c\sqrt{r}$ for polynomial growth $D > 2$ (\sproved) --- the additive
    $\tfrac{1}{2}$ of the cascade-exponent law is expander-specific, and the Heisenberg
    capacity-to-growth map on the growth branch is $\beta_{\mathrm{Heis}} = D - \delta_c$.
    Any continued use of the $1/(\delta + \tfrac{1}{2})$ form with Heisenberg-measured exponents
    therefore rests on a two-substrate identification, to be stated as a hypothesis.
    Window-effective exponents at $q \le 211$ sit near the marginal boundary $\delta_c \approx D$
    (observation on window-effective exponents; asymptotic branch open).
  ...
  [H-color] proved analytically via O31 v1.5).
    ...
    Confirmed numerically (\sstruct) for
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    The chain is conditional at its first link: Q5a version 3.0 withdraws the Mosco derivation
    of the spatial limit operator, which is now the explicit, unestablished hypothesis [H-L]
    (Q5b version 2.0); the geometric convergence results and algebraic rigidities are
    independent of [H-L]. Their individual contributions follow.

    \textbf{Q5a}~\cite{Beau2026q5a}: Version 3.0 (\emph{Canonical Fourier Filtration and the
    Obstruction to a Spatial Continuum Limit}) identifies the canonical filtration exactly ---
    ...
    conditional Dirichlet form in the rescaled \emph{frequency} variable.
    ...
    Version 2.0 formalises the spatial limit input as the explicit, unestablished
    hypothesis [H-L]; the Carnot/$D_{\mathrm{hom}}=4$ results are independent of it
    ...
    as the implication [H-F] $\Rightarrow$ [H-lift]; version 1.1).
    ...
    After W1, the open hypotheses of the former Q5a framework reduce to three: [H1], [H-E1],
    and [C].
    ...
    All hypotheses of the former Q5a convergence framework are thus verified on the admissible
    sector; the framework itself is withdrawn in Q5a version 3.0, and the chain's residual
    conditionality is the spatial limit hypothesis [H-L].
    ...
    U1 $\to$ W1 $\to$ H2 $\to$ Q11 $\to$ Q5b-O3 closed is complete \emph{conditionally on
    [H-L]} (Q5a version 3.0; Q5b version 2.0).
    ...
    Q12 is shared with the gauge-structure sub-programme (Note~3), which supplies the gauge
    group identification that Q12 takes as input; GGS-Note draws on Q12's $a_4$ vertical
    variation, on the
    ...
    GGS-Note~1.2 is aligned with Q12 and Q13: it records the fixed-metric isolation of the gauge
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{longtable}{@{}p{1.45cm}p{2.9cm}p{1.3cm}p{6.5cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      \label{tab:inventory}\\
      \toprule
      ...
      v1.16: continuum-limit passages conditioned on [H-L] (Q5a 3.0 withdrawal); co-metric
      completion conditional on [H-L]; relation to HJ, symplectic, WKB, FRG formalisms &
      \sproved \\[3pt]
      ...
      Verticality Lemma; transfer
      $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^* \approx 0.126$ unconditional
      w.r.t.\ the fibre structure of $\Pi$ (Cor.~4.2) &
      \sproved \\[3pt]
      \midrule
      ...
      $\delta_{\mathrm{corr}} \in [7.4,10.6]$ confirmed; $1\%$ threshold rank
      $r_{\mathrm{eff}}^{1\%}=3$ in $\mathrm{End}(H_{\mathrm{eff}})$, resolved spectrum
      ...
      \sproved \\[3pt]
      SGN & Span-growth classification (Heisenberg) & \cite{Beau2026sgn} &
      $\Delta r=\sigma_c|S_n|$ + exact $D=4$ spheres force growth/log/saturation branches in
      $(\delta_c,D)$; $r/|B_n|\to0$ for all $\delta_c>0$; no uniform $\sqrt{r}$ frontier for
      $D>2$, so the $+\tfrac12$ law is expander-specific; Heisenberg map
      $\beta_{\mathrm{Heis}}=D-\delta_c$ on the growth branch &
      \sproved \\[3pt]
      ...
      v3.0: canonical filtration identified exactly (toric Fourier window
      $\mathrm{span}\{e^{2\pi i b x/q}: |b|\le n\}$); published form converges to the zero form
      (rate $q^{-2}$); normalisation no-go (no common scalar normalisation yields a
      non-trivial toric differential operator); conditional dual-window Dirichlet limit in the
      rescaled frequency variable.
      Former Mosco derivation of $L_\Pi=-A\partial_x^2$ (v$\le$2.1) withdrawn; spatial limit
      now hypothesis [H-L]; Q5 open &
      \sproved{} (negative); \sopen{} (continuum) \\[3pt]
      ...
      closures apply to the withdrawn Q5a framework [H-F] &
      \sproved \\[3pt]
      Q5b & 4D Lorentzian geometry & \cite{Beau2026q5b} &
      BFS stratification of $\Heis_3(\mathbb{R}) \to$ 4D Lorentzian spacetime;
      $g^{\mu\nu}=2\,\eta^{\mu\nu}$ determined (Q9, Q10, Q11).
      v2.0: spatial limit input formalised as unestablished hypothesis [H-L]; metric results
      conditional on [H-L]; Carnot/$D_{\mathrm{hom}}=4$ geometry independent &
      \sstruct{} (geometry); conditional on [H-L] (metric) \\[3pt]
      ...
      [H-lift] closed by Q9; conditional on [H-L] &
      \sstruct \\[3pt]
      ...
      $A_H > 0$ proved by coercivity, independently of bridge.
      v1.1: the withdrawn Q5a framework is consumed as the frozen hypothesis [H-F]; the
      result is the implication [H-F] $\Rightarrow$ [H-lift] &
      \sproved \\[3pt]
      ...
      after W1 + Q5a-O2~\cite{Beau2026q5ao2}, open hypotheses of the former Q5a framework
      reduced to [H2] only &
      \sproved \\[3pt]
      ...
      closes last open hypothesis of the former Q5a framework (Theorem~7.1 of v$\le$2.1) &
      \sproved \\[3pt]
      ...
      $g^{\mu\nu} = \mathrm{diag}(-2,2,2,2) \propto \eta^{\mu\nu}$.
      v1.1: algebraic content unchanged; metric reading conditional on [H-L] &
      \sstruct \\[3pt]
      ...
      Joint $\delta_{g,\mathcal{A}} S^{\mathrm{ren}} = 0$ yields a conditional
  ...
  \subsection{Branch II (O-Series)}
    ...
    \paragraph{O29 at larger primes.}
      The carrier rank $r_{\mathrm{eff}} = 3$ is confirmed for
      $q \in \{61, 101, 151, 211\}$
  ...
  Branch~II (O-series) has closed the transfer chain $c_{\mathrm{BI}} \to \delta_{\mathrm{pair}} \to \beta^*$
  analytically~(O24) --- unconditionally with respect to the fibre structure of $\Pi$, with the
  O4--O6 growth-equation closure and coupling hypotheses, the Weil realisation, and the power-law
  regime as standing conditions --- and numerically validated it up to $q = 601$~(O25).
