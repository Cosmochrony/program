...
\subsection{O1--O8: LPS Phase --- Cascade Exponent \texorpdfstring{$\beta$}{beta}}
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    open.
    The geometric convergence results and the algebraic rigidities are independent of both.
    Their individual contributions follow.
    ...
    space inherits that structure and an equivariant map to the complexified adjoint is either zero
    or unique up to one non-zero complex scalar (reality and positivity being a separate requirement
    and then a convention).
    That the pullback of the Hermitian spatial form is graded carries four further premises: the
    existence of the bridge, the existence of the co-metric under [H-L] and [H-lift], the
    $J_3$-invariance of the admissible regime, and the positivity of both coefficients; under that
    invariance, isotropy is forced within the horizontal plane only.
    ...
    The entry $\tilde L_{00} = 4-2-0 = 2$, historically named $A_z$, is thus the flat-mode value: not
    a Casimir signature and not normalisation-invariant, and $A_z$ itself is left an unevaluated
    parameter.
    ...
    which differs from the stored-basis matrix at two pairs; the invariant content is its
    spectrum.
    ...
    this cascade.
    Q7 further records that the support was circular rather than merely absent, its scalar having
    been fixed by matching a value that was itself obtained by inserting the first.
