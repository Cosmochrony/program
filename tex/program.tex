...
\begin{document}
  ...
  \paragraph{Conventions on result status.}
    ...
    item, the status cell records it as a secondary label: a proved no-go or countermodel is \sproved{} as a
...
\subsection{O1--O8: LPS Phase --- Cascade Exponent \texorpdfstring{$\beta$}{beta}}
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    (\scond{} on the Q5a Mosco hypotheses and [H-L], conditional on [H-L]).
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
      ...
      Colour coding: \textcolor{cproved}{$\blacksquare$}~\sproved{},
        ...
        In both figures the grey side annotations carry no status colour: where their text states a status it is
        that of the component named, not the primary label of the node.
    ...
    \begin{figure}[p]
      ...
      \textbf{Q9}~(\scond{} on [H-F]; the implication itself is \sproved): under the frozen, unestablished
        ...
        \textbf{Q8}~(\sstruct{}, within the frozen [H-F]): $A_Z = C_{\mathfrak{su}(2)} = 2$ by Casimir
        rigidity; normalisation fixed by horizontal sector.
