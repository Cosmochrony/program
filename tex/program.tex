...
\date{2026\\[2pt]\small Version 2.13}
...
\begin{document}
  ...
  \subsection{O16--O24: Pair Observable and the Fibre-Level Structure Programme}
    ...
    and $\mathrm{SU}(2)$-valued (\sstruct{}: exact computation on deposited witnesses with reproduction code, not a universal claim).
  ...
  \subsection{O25--O30, PTO, and Fibre Erasure: Numerical Validation, Sector Identification, Eigenvalue Ratio, and Information-Theoretic Axes}
    ...
    Thus, within the supplied three-coordinate carrier ($H_{\mathrm{eff}} = \mathbb{C}^3$
    is the $\mathrm{HEFF\_DIM}=3$ truncation, whose ambient rank is not measurable from
    these coordinates), $r_{\mathrm{eff}} = 3$ characterises the truncated data as the
    irreducible adjoint module
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    \textbf{Q7}~\cite{Beau2026q7}: Bridge candidate between the supplied admissible
    dimension $\dim H_{\mathrm{eff}} = 3$ (the O23 selection rule, implemented as
    $\mathrm{HEFF\_DIM}=3$ in the O25--O29 pipeline) and the three spatial dimensions of
    Q5b.
