...
\subsection{O1--O8: LPS Phase --- Cascade Exponent \texorpdfstring{$\beta$}{beta}}
    ...
    that the paper's own fingerprint does not satisfy (\scond\ /\ \sopen).
    ...
    pre-saturation window to $O(\log q)$ BFS steps (\snum).
  ...
  \subsection{O9--O15: Heisenberg Transition}
    ...
    Confirms $n^4$ growth; a TensorSketch representation obstruction is identified at large~$q$ (\snum).
  ...
  \subsection{O16--O24: Pair Observable and the Fibre-Level Structure Programme}
    ...
    \textbf{O16}~\cite{Beau2026a20}: Fibre admissibility hypotheses (\scond).
    ...
    itself are open; the numerical interval $[7.4,10.6]$ is not derived (\scond).
    ...
    (\scond{} under the Weil realisation and stated fibre hypotheses).
  ...
  \subsection{O25--O30, PTO, and Fibre Erasure: Numerical Validation, Carrier Diagnostics,
    ...
    spectral cascade (\scond). Defines a residual fibre-information functional $I(c;\sigma(\ell))$ on the
  ...
  \subsection{Q-Series: Quantum Mechanics and Spacetime Emergence}
    ...
    hypothesis (\scond): given a supplied bipartite carrier $V_{\chi_{2j+1}}\otimes
  ...
  sub-programme}, whose synthesis paper is the presentation note \textbf{EG-Note}~\cite{Beau2026egnote}.
    ...
    on supplied geometric and gauge data (\scond, conditional on Q12, the Gravity paper,
  ...
  \subsection{Gravity and Spacetime Track}
    ...
    $S_\Pi[g] = \tfrac{1}{2}\log\det' A_g$ supports a local infrared Einstein sector (\scond). It separates two
    ...
    Establishes a conditional variational core, and no thermodynamics (\scond).
    ...
    is not available --- an in-in construction on a closed time path is required and is \sproved{} --- so the local
  ...
  \subsection{Lorentz Capacity Sub-Programme}
    ...
    Born--Infeld admissibility budget (\scond). The analytic factorization (central coordinate $c$
    ...
    $\mathcal{P}_\tau^{\mathrm{Q5b}}$ from LorentzCapacity (\scond).
    ...
    \textbf{LCII}~\cite{Beau2026lco2}: Extends TempProj to the non-homogeneous fibrewise regime (\scond).
  ...
  \subsection{Standard Model Companion Papers}
    ...
    diagonal-invariance hypothesis (H-inv) (\scond). Q1 does not establish a singlet correlator
...
% aggregates Q14, PRS, AAR, Q11OF, A4-Note and AOG-Note.
    ...
    \begin{figure}[p]
      ...
      \resizebox{\textwidth}{!}{%
        \begin{tikzpicture}[
          ...
          \node[C] (Q10) at (\dx,-4*\dy) {Q10\\{\tiny $A_H\!\to\!2$}};
          \node[C] (U1)  at (\dx,-5*\dy) {U1\\{\tiny [U] under Q5a--Q5b hyp.}};
          \node[C] (W1)  at (2*\dx,-5*\dy){W1\\{\tiny [H-w] under [H1],[H2]}};
    ...
    \begin{longtable}{@{}p{1.70cm}p{2.9cm}p{1.3cm}p{6.25cm}>{\raggedright\arraybackslash}p{2.0cm}@{}}
      ...
      \scond{} on the capacity--isoperimetry closure hypothesis \\[3pt]
      \midrule
      ...
      W1 & [H-w] under [H1] and [H2] & \cite{Beau2026w1} &
      ...
      proves hypothesis [H2] of the former Q5a framework, whose [H-E1] and [C] are not established &
      \sproved \\[3pt]
      ...
      \sproved{} (negative, about the posited truncation); \sopen{} (Lorentzian construction) \\[3pt]
      ...
      projective chirality (Hopf fibration) established at the framework level, qualitatively &
      \sproved \\[3pt]
